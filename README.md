# 👋 Hi, I'm Joyal K Shajan

### `Data Analyst` • `AI/ML Enthusiast` • `MCA Student` • `Python Developer`

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=2800&pause=900&color=58A6FF&center=true&vCenter=true&width=700&lines=Turning+Data+into+Insights+%F0%9F%93%8A;Building+AI-Powered+Solutions+%F0%9F%A4%96;Exploring+Machine+Learning+%F0%9F%A7%A0;MCA+Student+%7C+Data+Analytics+Enthusiast+%F0%9F%9A%80" alt="Typing Animation" />

<p align="center">
  <a href="https://github.com/">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
 <img src="https://komarev.com/ghpvc/?username=joyalkshajan-leo&style=for-the-badge&color=blue" alt="Profile Views"/>
</p>

---

## 🧑‍💻 About Me

I'm **Joyal K Shajan**, an MCA student with a strong interest in **Data Analytics, Artificial Intelligence, Machine Learning, and Python development**.

I enjoy transforming raw data into meaningful insights and building intelligent solutions that solve real-world problems.

* 🎓 Currently pursuing **MCA**
* 📊 Aspiring **Data Analyst**
* 🤖 Exploring **AI & Machine Learning**
* 🐍 Python enthusiast
* 📈 Interested in **Data Visualization & Business Intelligence**
* 🧠 Learning **Agentic AI & intelligent systems**
* 🚀 Building projects that combine **data + AI + automation**
* ☕ Always learning something new

> **"Turning data into insights. Turning ideas into intelligent solutions."**

---

## ⚡ Tech Stack

### 💻 Programming & Development

<p>
<img src="https://skillicons.dev/icons?i=python,java,c,html,css,js,django,sqlite,git,github,vscode" />
</p>

### 📊 Data & Analytics

<p>
<img src="https://skillicons.dev/icons?i=python" />
</p>

`Pandas` • `NumPy` • `Matplotlib` • `Seaborn` • `Excel` • `SQL`

### 🤖 AI / Machine Learning

`Machine Learning` • `Artificial Intelligence` • `Data Preprocessing` • `Classification` • `Prediction` • `Model Evaluation`

### 🛠️ Tools & Platforms

<p>
<img src="https://skillicons.dev/icons?i=github,git,vscode,googlecolab,docker" />
</p>

---

## 🚀 Current Projects

### 🏥 Smart Healthcare Analytics

> Exploring AI-driven healthcare systems for **personalized treatment analysis, medical data insights, and intelligent decision support**.

**Focus:**
`Healthcare Analytics` `Machine Learning` `AI` `Data Visualization`

---

### 🧠 AI & Data Analytics Projects

Currently exploring projects involving:

* 📊 Data analysis & visualization
* 🤖 Machine learning prediction
* 🧬 Healthcare AI
* 🔍 Explainable AI
* 🧠 Agentic AI
* ⚙️ Workflow automation
* 📚 RAG-based intelligent systems

---

## 📌 Featured Areas

```text
Data Analytics       ███████████████████░░   90%
Python               ██████████████████░░░   85%
Machine Learning     ████████████████░░░░░   80%
AI                   ███████████████░░░░░░   75%
Web Development      ██████████████░░░░░░░   70%
SQL                  ██████████████░░░░░░░   70%
```

---

## 📊 GitHub Analytics

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&hide_border=true&count_private=true&include_all_commits=true&theme=transparent" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&hide_border=true&theme=transparent" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&hide_border=true&theme=transparent" />
</p>

---

## 🧰 Tools I Work With

| Category         | Tools                         |
| ---------------- | ----------------------------- |
| 🐍 Programming   | Python, C, Java               |
| 📊 Analytics     | Pandas, NumPy, Excel          |
| 📈 Visualization | Matplotlib, Seaborn           |
| 🗄️ Database     | SQLite, SQL                   |
| 🤖 AI/ML         | Machine Learning, AI          |
| 🌐 Web           | HTML, CSS, JavaScript, Django |
| 🔧 Development   | Git, GitHub, VS Code          |
| ⚡ Automation     | n8n                           |
| ☁️ Platforms     | Google Colab                  |

---

## 📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME&hide_border=true&area=true&theme=github-compact" />
</p>

---

## 🤝 Connect With Me

<p align="center">

<a href="https://www.linkedin.com/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/">
<img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</p>

---

## 💡 What I'm Working Toward

```text
                    ┌──────────────────────┐
                    │       RAW DATA       │
                    └──────────┬───────────┘
                               ↓
                    ┌──────────────────────┐
                    │   DATA ANALYTICS     │
                    └──────────┬───────────┘
                               ↓
                    ┌──────────────────────┐
                    │   MACHINE LEARNING   │
                    └──────────┬───────────┘
                               ↓
                    ┌──────────────────────┐
                    │     AI SOLUTIONS     │
                    └──────────┬───────────┘
                               ↓
                    ┌──────────────────────┐
                    │  REAL-WORLD IMPACT   │
                    └──────────────────────┘
```

### 🎯 Career Goal

**Data Analyst → Data Scientist → AI/ML Engineer**

I'm continuously improving my skills in **data analysis, machine learning, AI, visualization, and intelligent automation** to build solutions that create measurable real-world impact.

---

<p align="center">

### ⭐ If you find my projects interesting, consider giving them a star!

**Thanks for visiting my profile! 🚀**

</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>
</p>
